    ...
    vec![
        ...
        VehicleSection::new(
            ...
            vec![
                VehicleField::new(
                    "Yaw Rate",
                    format!("{:.2}", (elapsed * 0.9).sin() * 6.0),
                    Some("°/s"),
                ),
                VehicleField::new(
                    "Pitch Rate",
                    format!("{:.2}", (elapsed * 0.7).cos() * 3.0),
                    Some("°/s"),
                ),
                VehicleField::new(
                    "Roll Rate",
                    format!("{:.2}", (elapsed * 0.4).sin() * 2.0),
                    Some("°/s"),
                ),
                VehicleField::new(
                    "Suspension FL",
...
fn build_metrics(elapsed: f32, torque: f32, battery: f32) -> Vec<DerivedMetric> {
    ...
    vec![
        ...
        DerivedMetric::new(
            ...
            format!(
                "{:.2}",
                efficiency_spark.last().copied().unwrap_or(3.2).max(2.3)
            ),
            efficiency_windows,
            efficiency_spark,
        ),
