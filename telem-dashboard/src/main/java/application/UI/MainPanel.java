...
public MainPanel(TelemetryLookup lookup, int chartCountVertical, int chartCountHorizontal) {
    ...
    for (var entry : seriesByRef.entrySet()) {
        ...
        TelemetryRecords.DataInfo info;
        try {
            info = lookup.getDataInfo(key);
    ...
    DropTargetListener dtl = new DropTargetAdapter() {
        ...
        public void drop(DropTargetDropEvent dtde) {
            try {
                ...
                lookup.getDataInfo(key); // Check that it exists, will throw if not.
    ...
    private void applyThresholdMarkers(JFreeChart chart, TelemetryLookup.DataKey key) {
        try{
            TelemetryRecords.DataInfo dp = lookup.getDataInfo(key);
            ...
            System.out.println("unable to find dataInfo for a chart that needs to be displayed: " + e.getMessage());
        }
