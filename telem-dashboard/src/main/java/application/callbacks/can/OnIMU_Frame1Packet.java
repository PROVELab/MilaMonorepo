...
public class OnIMU_Frame1Packet {

    public void handle(CANFrameParser.IMU_Frame1Packet p, MainPanel mainPanel, NotificationPanel notifications, TelemetryLookup lookup) {
        ...
        // You can access the data via p.dataName() methods, e.g., p.posX_m()

        PositionGraphing graph = PositionGraphing.getInstance();    //Gets the graph created in mainApp
        if(graph != null){
            PositionGraphing.plotIMU(graph, p.posX_m(), p.posY_m(), p.posZ_m());
        }
