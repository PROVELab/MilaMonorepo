...
int16_t recieveHeartbeat(CANPacket* message) { // mark the HB for given node as recieved, recording time to respond
    ESP_LOGD(TAG, "Received Pong from: %lu", (message->id) & 0x7F);
    int16_t nodeIndex = IDTovitalsIndex(message->id);
    if (nodeIndex == invalidVitalsIndex) {
        ESP_LOGW(TAG, "Received HB from invalid nodeId %lu, ignoring", message->id);
        return 0; // invalid id
    }
...
static inline void updateSlowestIndex(uint32_t* slowestNodesArray, int16_t* worstTimesArray, uint8_t nodeIndex, int16_t time) {
    for (int i = 0; i < 3; i++) {
...
static void checkHB(void* pvParameters) {
    ...
    for (;;) {
        ...
        sendHBStatusArgs status_args = { .HBMask = hb_mask };
        sendHBStatusFunction(status_args);

        // Send HBTiming packet
        sendHBTimingArgs timing_args = {
            .slowestNode1_ID = (worstTimes[0] > 0) ? vitalsIndexToID(slowestNodeIndices[0]) : 0,
            .slowestNode1_time = worstTimes[0],
            .slowestNode2_ID = (worstTimes[1] > 0) ? vitalsIndexToID(slowestNodeIndices[1]) : 0,
            .slowestNode2_time = worstTimes[1],
            .slowestNode3_ID = (worstTimes[2] > 0) ? vitalsIndexToID(slowestNodeIndices[2]) : 0,
...
static void printAllData() { // not for final use. for testing only
    for (int i = 0; i < numberOfNodes; i++) { // each node
        ESP_LOGD(TAG, "Node %d (ID: %d), numFrames: %d", i, vitalsIndexToID(i), (nodes[i]).numFrames);
