...
static bool processFoundPacket(const RecvPacketLUTEntry* entry, const uint8_t*& stream_ptr, size_t& remaining_len) {
    ESP_LOGD(TAG, "Found packet with mask: %" PRIu32 " (0x%" PRIX32 "), bits: %u", entry->mask_val, entry->mask_val, entry->mask_bits);
    ...
    if (entry->callback_wrapper) {
        ...
        ESP_LOGW(TAG, "No callback wrapper defined for packet with mask: %" PRIu32 " (0x%" PRIX32 ")", entry->mask_val, entry->mask_val);
    }
...
static bool parseNextMessage(const uint8_t*& stream_ptr, size_t& remaining_len, const uint8_t* stream_start) {
    ...
    for (int i = recvPacketLUTSize - 1; i >= 0; --i) {
        ...
        if (entry->mask_bits > 0 && entry->mask_bits <= remaining_len * 8) {
            // Determine how many bytes we need to read to check this specific mask.
            const size_t bytes_needed_for_mask = (entry->mask_bits + 7) / 8;

            if (remaining_len >= bytes_needed_for_mask) {
                // Safely copy just enough bytes to check the mask.
                // Initialize to 0 to avoid reading garbage in upper bytes if we copy < 4 bytes.
                uint32_t stream_val = 0;
                memcpy(&stream_val, stream_ptr, bytes_needed_for_mask);

                // Apply the bitmask and compare.
                // Note: This assumes the stream is little-endian, which is true for our system.
                const uint32_t current_mask = stream_val & ((1UL << entry->mask_bits) - 1);
    ...
    int8_t* telem_bit_idx_ptr)
{
    ...
    if (num_fields > 0 && args_data != NULL) {
        for (int i = 0; i < num_fields; ++i) {
            const simpleDataPoint* field_info = &fields[i];
            pecan_pack(can_p.data, &can_bit_idx, args_data[i], field_info);
        }
    }
